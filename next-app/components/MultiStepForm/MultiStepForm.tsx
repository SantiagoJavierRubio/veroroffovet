...
export default function MultiStepForm(props: MultiStepFormProps) {
  ...
  return (
    ...
    >
        ...
        {step}
        <div className='flex justify-between px-8 max-w-4xl m-auto mt-4'>
            {!isFirstStep() ? 
                <button type="button" id="backBtn" onClick={back}>Atras</button>
                :
                <div></div>
            }
            {isLastStep() ? 
                <button type="submit" id="finalSubmitBtn">Finalizar</button> 
                : 
                <button type="submit" id="continueBtn">Seguir</button>
            }
        </div>
    </form>
  )
